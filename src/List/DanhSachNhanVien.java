...
public class DanhSachNhanVien implements InterfaceDocGhi {
	...
	public static void xuatdsnv() {
		...
		System.out.format(
				"+-----------------+----------------------+-----------------+--------------------------------+-----------------+----------------------+-----------------+--------------------------------+\n");
		System.out.format(format, "Mã nhân viên", "Họ nhân viên", "Tên nhân viên", "Lương", "Chức vụ", "Số điện thoại",
				"Số căn cước", "Địa chỉ");
		System.out.format(
				"+-----------------+----------------------+-----------------+--------------------------------+-----------------+----------------------+-----------------+--------------------------------+\n");
		for (int i = 0; i < dsnv.length; i = i + 1)
			System.out.format(format, dsnv[i].getMaNv(), dsnv[i].getHoNv(), dsnv[i].getTenNv(),
					df.format(dsnv[i].getLuong()), dsnv[i].getChucVu(), dsnv[i].getSdtNv(), dsnv[i].getSoCanCuoc(),
					dsnv[i].getDiaChiNv());
		System.out.format(
				"+-----------------+----------------------+-----------------+--------------------------------+-----------------+----------------------+-----------------+--------------------------------+\n");
	}
	...
	public void ghiFile() {
		...
		try {
			...
			for (NhanVien nv : dsnv) {
				if (nv != null) {
					String data = nv.getMaNv() + ";" + nv.getHoNv() + ";" + nv.getTenNv() + ";" + nv.getLuong() + ";"
							+ nv.getChucVu() + ";" + nv.getSdtNv() + ";" + nv.getSoCanCuoc() + ";" + nv.getDiaChiNv()
							+ "\n";
