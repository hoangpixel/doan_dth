...
public class DanhSachChiTietHoaDon implements InterfaceDocGhi {
						...
						do {
							...
							if (dt != null) {
								if (dt.getSoluong() < soLuong) {
									System.out.println("Số lượng điện thoại còn trong kho không đủ, chỉ còn "
											+ dt.getSoluong() + " chiếc, vui lòng nhập số lượng khác ");
									quan = false;
								}
							}
						} while (!quan);
	...
	public void ghiFile() {
		...
		try (BufferedWriter writer = new BufferedWriter(new FileWriter(filePath))) {
			for (ChiTietHoaDon ct : cthd) {
				writer.write(ct.getMahd() + "," + ct.getMadt() + "," + ct.getSoluong() + "," + ct.getDongia() + ","
						+ ct.getThanhtien());
				writer.newLine();
			}
