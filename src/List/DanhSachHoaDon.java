...
public class DanhSachHoaDon implements InterfaceDocGhi {
	...
	public void nhapDanhSachHoaDon(DanhSachNhanVien temp1, DSKhachHang temp2, NhanVien temp3) {
		...
		if (danhsachmakh.length == 0 || danhsachmanv.length == 0) {
			System.out.println(
					"Danh sách nhân viên hay danh sách khách hàng trống,"
					+ " hãy nhập danh sách nhân viên hay danh sách khách hàng trước");
			return;
		}
		for (int i = 0; i < n; i = i + 1) {
			dshd[i] = new HoaDon();
	...
	public void xuatDanhSachHoaDon() {
		...
		System.out.format(
				"+-----------------+----------------------+-----------------+-----------------+-----------------+\n");
		for (int i = 0; i < n; i = i + 1)
			System.out.format(format, dshd[i].getMaHd(), dshd[i].getNgayLapHd(), dshd[i].getMaNv(), dshd[i].getMaKh(),
					df.format(dshd[i].getTongTien()));
		System.out.format(
				"+-----------------+----------------------+-----------------+-----------------+-----------------+\n");
	}
...
//Hàm thêm hoá đơn, lấy mã nhân viên từ mã nhân viên login, lấy mã khách hàng từ danh sách khách hàng
	public void themHoaDon(DanhSachNhanVien temp1, DSKhachHang temp2, NhanVien temp3) {
		...
		for (int i = 0; i < add; i = i + 1) {
			...
			dshd[n] = new HoaDon();
			dshd[n].nhap(temp3.getMaNv(), danhsachmakh); // Đưa mã nhân viên, danh sách mã khách hàng vào hàm nhập bên
															// hoá đơn
	...
	public void xoaTheoMa() {
		...
		int position = timTheoMa(ma); // Tìm vị trí của mã hoá đơn muốn xoá
	...
	public static HoaDon timTheoMaHD(String maHD) {
	...
	public void timTheoMa() {
		...
		for (int i = 0; i < n; i = i + 1)
			if (dshd[i].getMaHd().equals(ma)) {
				System.out.println("Vị trí của hoá đơn cần tìm là: " + (i));
				dshd[i].xuat();
				return;
			}
	...
	// Hàm tìm theo mã hóa đơn trả về mã khách hàng trong hóa đơn đó
	public static String timMaKH(String maHD) {
		for(HoaDon hd : dshd) {
	...
	public void ghiFile() {
		...
		try {
			...
			for (HoaDon hd : dshd) {
				...
				+ hd.getMaNv() + ";" + hd.getMaKh() + ";" + hd.getTongTien() + "\n";
...
// VD input: 3, 2023; output: 2
	public int kiemtraQuy(int quy, int nam) {
		...
		for (int i = 0; i < n; i = i + 1) {
			if (quy == 1) {
				if ((dshd[i].getMonth() == 1 || dshd[i].getMonth() == 2 || dshd[i].getMonth() == 3)
						&& dshd[i].getYear() == nam)
					dai = dai + 1;
			}
			if (quy == 2) {
				if ((dshd[i].getMonth() == 4 || dshd[i].getMonth() == 5 || dshd[i].getMonth() == 6)
						&& dshd[i].getYear() == nam)
					dai = dai + 1;
			}
			if (quy == 3) {
				if ((dshd[i].getMonth() == 7 || dshd[i].getMonth() == 8 || dshd[i].getMonth() == 9)
						&& dshd[i].getYear() == nam)
					dai = dai + 1;
			}
			if (quy == 4) {
				if ((dshd[i].getMonth() == 10 || dshd[i].getMonth() == 11 || dshd[i].getMonth() == 12)
						&& dshd[i].getYear() == nam)
					dai = dai + 1;
			}
...
// Hàm tạo mảng năm
	public int[] taoMangNam() {
		...
		int[] year = Arrays.copyOf(nam, dai); // Vì khởi tạo mảng năm với 100 phần tử nên sẽ có phần tử rỗng, nên copy
												// mảng năm thành mảng năm có dai phân tử, rồi return về
		return year;
	}
...
// Hàm tạo mảng quý của một năm, lưu các hoá đơn trong một quý của một năm
	public HoaDon[] quyII(int nam) {
		...
		int j = 0;
		for (int i = 0; i < n; i = i + 1)
			if ((dshd[i].getMonth() == 3 || dshd[i].getMonth() == 4 || dshd[i].getMonth() == 5)
					&& dshd[i].getYear() == nam) {
				quyII[j] = dshd[i];
				j = j + 1;
			}
...
// Hàm tạo mảng quý của một năm, lưu các hoá đơn trong một quý của một năm
	...
	// Hàm cập nhật lại tổng tiền đã mua của khách hàng có mã xác định
	public static void thaydoitienKH(String makh){
        float tongtien = 0;
        for(HoaDon hd : dshd) {
        ...
        KhachHang kh = DSKhachHang.timkiemMaKH(makh);
